...
class zipper {
		...
		if (is_dir($source) === true) {
			...
			foreach ($files as $file) {
				...
				if ($file !== "." && $file !== "..") {
					...
					if (is_dir($file) === true) {
						$a[] = array('type' => 'dir', 'source' => str_replace($source . '/', '', $file . '/'), 'file' => $file, 'size' => 0);
					} else if (is_file($file) === true) {
						$src = str_replace($source . '/', '', $file);
						$size = filesize($file);

						$a[] = array('type' => 'file', 'source' => $src, 'file' => $file, 'size' => false != $size ? $size : 16000 // this is fallback in case no size
						);
					}
				}
			}
	...
	public function ProcessZip($foldercontent, $folder, $maxsize) {
		...
		if (isset($foldercontent)) {
			...
			return array('splits' => count($split), 'foldername' => '');
		}
	}
